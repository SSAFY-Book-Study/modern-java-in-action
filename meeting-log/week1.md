...
---
## 3번 이슈 [람다는 로컬 변수에 접근하는 기능이 생겨서는 안되는 걸까요]
1. 안건을 올린 이유 발표
- 다양하게 쓸 수 있을 것 같아서~~~
...
1. 안건을 올린 이유
-  Synchronized 방식이 효율이 좋지 않다고 들었는데, Parallel Stream을 이용하면서 더 발전이 된 것인지, 쓰임에 차이가 있는 것인지 궁금해서 이슈에 올렸습니다.

2. 안건에 대해 조사한 사람이 설명
- Parallel stream을 쓴다고 해서 Synchronized 사용이 완전히 제한되는 것은 아님
- Synchronized는 Collecitons에서 쓰지 않음! 하나의 스트림을 처리하는 것이기 때문에 Sync를 사용할 필요가 없음. Collections Stream에서는 Parallel Stream을 쓰는게 더 나음
- Parallel Stream은 완벽히 독립적으로 분할이 가능한 작업이어야 이득이 있음

- Synchronized 문제?
> Synchronized는 단순히 한명만 쓸 수 있게 막는 것, 블로킹! 스레드를 할당했지만 블로킹 되어 있으므로 시간, 자원 상으로 손해, Mutex!
자원의 손실과 성능의 하락이 큼

- Parallel Stream에서의 동시성 처리 : 주어진 일을 완벽하게 독립적으로 영향이 가지 않을 때 까지 분해하여 각자 처리하여 수행. -> 분할 정복! 

3. Q n A
- Synchronized는 언제 쓸까? -> 강제적으로 스레드를 하나만 접근할 수 있도록 막음
  > Stream에서 상태를 건들 때 써야하는게 아닐까? -> 정말 동시성에 중요한 공유 락!
- Singleton pattern을 사용할 때 여러 곳에서 접근할 수 있어서 사용할 수 있음
  > holder 방식으로 해결하면 JVM 선에서 동시성 처리가 됨.
- static inner class는 바로 메모리에 올라가지 않고, 호출된 순간에 메모리에 올라감 -> Lazy loading

---
## 9번 이슈 람다는 클로저인가? 자바에서 클로저란?
1. 안건을 올린 이유
- 클로저에 대해 알고가고 싶어서.
...
- 클로저는 외부 변수를 사용하는 것
- 외부 변수를 함수 안에서 사용한 것
- 책에서는 외부 변수를 가져다 쓰는게 클로저와 유사하다고 표현한 것 같다.
- 자바에서 클로저는 람다식이나 내부 클래스 같은 영역 안에서 외부에서 정의된 변수를 사용하는 방식을 의미한다.
- 자바에서 람다식을 람다식 외부에서 정의된 변수가 사용될 경우 클로저가 사용됐다고 볼 수 있다.
- 
3. Q n A
- 자바스크립트에서 클로저는 어떤가요?
  > 외부함수(포함하고 있는)의 변수에 접근할 수 있는 내부 함수입니다.

- 클로저는 자바스크립트의 고유한 개념이 아니라, 여러 함수형 프로그래밍 언어에서 공통적으로 발견되는 특성
