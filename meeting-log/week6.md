...
2. 안건에 대해 답변자 발표 및 설명 요약
- 제네릭의 타입 안정성 검사
  - 제네릭을 사용한 경우 컴파일러는 타입 안정성을 검사하기 위해 타입소거라는 과정을 진행함
  - 런타임에는 제네릭이 어떤 타입을 가지는지 알 수 없음
 	- 컴파일 타입이 결정되기 때문에 예외 발생 가능성이 낮음
  - 기본형을 사용한 경우 상대적으로 덜 엄격하게 검사하기 때문에 런타임에도 타입소거를 진행하지 않음
  	- 타입을 잘못 할당한 경우 ClassCastException이 발생할 수 있다.
3. Q n A 및 회의록 기록
---
