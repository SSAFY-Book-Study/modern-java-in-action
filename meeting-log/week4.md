...
1. 안건을 올린 이유 발표
   ...
   다른 Collection 등을 저장하는 Optional과 달리 거의 사용하는 예도 주어지지 않지만 존재는 한다고 책에서 나왔는데, 어떻게 사용하는 것이 바람직한 지에 대해서 궁금했습니다.
  
2. 안건에 대해 답변자 발표 및 설명 요약
   기본형은 애초에 null 값을 할당을 할 수가 없다.
   또한 Optional 기본형은 
   사용할 때 언박싱하는 작업이 필요
   기본형을 사용함으로써 래핑 언래핑 연산을 하지 않아도 된다.
   불편함을 감수하고도 기능을 지원하는 이유는
   특화 스트림에서 이를 사용하기 위함.
   OptionalInt를 사용하는 예시도 답변에 주어졌다.
