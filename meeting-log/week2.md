...
---
## 이슈 [Collectors.method() vs Stream.method()]
1. 안건을 올린 이유 발표
...
---
## 이슈 [코딩테스트에서 Stream이나 ParallelStream을 사용하지 않는 이유]
1. 안건을 올린 이유 발표
...
## 이슈 reduce를 collect처럼 사용할 때의 문제
1. 안건을 올린 이유 발표
- reduce를 collect처럼 사용할 수 있는 것 같은데, 그렇게 사용하기를 지양하라는 말은 있었자만...

2. 안건에 대해 답변자 발표 및 설명 요약
- 어떻게 연산을 하는지에 대한 답변을 의미론 적인 방법과 기능적 문제에 대해서 설명했습니다.
- collect는 스트링빌더 어레이리스트 등을 생성해서 그곳에 축적을 하는 방식으로 사용합니다.
- 리듀스는 인트값 등을 결합해서 새로은 값을 만들고 String 등을 사용해서 초기값에 축적하는 방식으로 사용합니다.
- 리듀스는 수학적 연산이라고 생각하면 되고, 콜렉터는 append를 연속해서 사용하는 식으로 사용한다고 합니다.
- 의미론적인 문제 : 초기값과 연산이 주의해야할 문제입니다.
  - 리듀스는 값이 계속 바뀌기에 항등원을 사용해줘야 합니다. 덧셈일 경우 0 곱셈일 경우 1
- 기능적인 문제 : 가변성 병렬
  - 병렬 값에 동시 접근을 하면 값이 바뀔 수 있다. 동기화가 필요할 수 있다.
- 리듀스 연산을 다 하고 collect에 넣어야 합니다.
