...
2. 안건에 대해 답변자 발표 및 설명 요약<br>
- 디자인패턴을 사용하는 이유
	- 특정 문제를 해결할 때 코드를 형식화한 것
	- 오픈소스를 잘 이해하기 위해서
- GoF
	- **생성패턴**<br>
	: 객체를 생성할 때 어떻게 잘 만들것인가?
		- 싱글톤 패턴
    			- 클래스에 인스턴스가 1개임을 보장하고 싶을때 사용
    			- 인스턴스 생성을 제어할 수 있고 지연로딩 도입
		- 빌더 패턴
    			- 생성자의 파라미터가 많을 때 순서파악 등 고려할 사항이 많아 캡슐화하고 싶을 때 사용
    			- 주로 롬복의 빌더패턴을 사용
		- 팩토리 패턴
  			- 객체 생성을 캡슐화하여 다른 클래스에게 객체 생성을 위임할 때 사용
       
	- **구조패턴**<br>
	: 소프트웨어를 설계할 때 어떻게 더 잘 만들 수 있을까?
		- 어댑터
  			-  c->8핀 이런식으로 인터페이스를 사용할때 
		- 파사드
  			- 로직을 캡슐화해서 따로 메서드로 빼는 방식
		- 데코레이터
  			- 기존 객체 기능을 확장할 경우 기존 객체를 상속받는 모든 객체 또한 모두 바꿔줘야하기 때문에 기존 객체를 감싸고 있는 새로운 객체를 만드는 패턴 
		- 프록시
  			- 데코레이터와 비슷하고 원래 객체를 감싸고있는 큰 클래스를 만들어서 액세스를 제어함
...
---
## 이슈 [map.of vs map.ofentries]
1. 안건을 올린 이유 발표
- 책에서 설명이 부실함을 느껴서 자세히 알고 싶었습니다.
...
- Map.ofEntries
	- 인자 가변인수라 10개 이상 가능 
- 어느 상황에 써야 할까?
	- 개수가 10개를 넘을 때는 Map.ofEntries 아니면 Map.of
